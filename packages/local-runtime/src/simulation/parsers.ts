...
export function parseAttributes(text:string):AttributeCandidate[]{
 ...
 for(const clause of s.split(/[;\n]|\.(?!\d)/u)){
  ...
  for(const m of clause.matchAll(/(\d+(?:[.,]\d+)?)\s*[- ]?\s*(?:inch(?:es)?\b|pouces?\b|\bin\b)/giu))out.push({attribute:'inches',value:m[1]!.replace(',','.'),unit:'inch',negated:neg(m.index),source:m[0]});
 }
 return out;
}
