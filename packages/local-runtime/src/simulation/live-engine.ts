...
export function validateLiveBinding(value:unknown,pack:DataPack):LiveBinding {
 ...
 validateParameters(b.config.parameters);const proposal=suggestConfig({instruction:b.config.instruction,hard_rules:b.hard_rules,interpretation:{},mandate_id:b.live_mandate_id,version:b.config.mandate_version} as MandateRecord,b.config.created_at);if(proposal.requirements.some(r=>r.filter_ids.includes('G06')&&r.parameter_keys.length===0))throw Error('live_unsupported_rule');assertNoWeakening(proposal.parameters,b.config.parameters);if(b.history_hash!==hash(pack.history)||b.pack_version!==pack.pack_version)throw Error('live_reference_pack_changed');return structuredClone(b);
}
/** Pure projection: only remote final approvals become commitments. Unknown submissions reserve. */
export function evaluateLive(pack:DataPack,binding:LiveBinding,event:AuthorizationEvent,runId:string,entries:LiveEntry[],now:string,answers:HumanAnswer[]=[],habitEntries:readonly LiveEntry[]=entries):Evaluation {
 ...
 for(const entry of entries){if(entry.id===event.authorization.authorization_id)continue;const a=entry.event.authorization;const ledger={authorization_id:entry.id,amount_chf:String(a.billing_amount_chf),timestamp:a.timestamp,quantity:a.items.reduce((n,i)=>n+i.quantity,0),budget_scope_id:runId};
  ...
  else if(entry.reserved)run.reservations.push({...ledger,offer_hash:offerHash(entry.event,config),expires_at:entry.human_expires_at&&entry.state==='awaiting_human'?entry.human_expires_at:'9999-12-31T00:00:00.000Z'});
 ...
 const behavior_profile=config.parameters.learn_confirmed_habits===true?safeBehaviorProfile(liveHabitObservations(habitEntries),{customerId:event.mandate.customer_id,scope:'live',asOf:event.authorization.timestamp,timezone:config.parameters.timezone}):undefined;
 const a=assess({behavior_scope:'live',...(behavior_profile?{behavior_profile}:{}),pack,event,config,run,now,offer_hash:offerHash(event,config),answers,phase:'commit'});
