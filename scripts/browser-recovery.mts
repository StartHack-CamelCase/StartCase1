...
import { configuredInstructionDecoder } from '../tests/helpers/configured-instruction-decoder.js';
const dir=await mkdtemp(join(tmpdir(),'viseca-browser-'));
// Test fixtures only: the browser exercises the normal asynchronous review flow.
const app=await createLocalApp({stateDir:join(dir,'state'),outputDir:join(dir,'output'),webDir:resolve('dist/apps/local-web/web'),instructionDecoder:configuredInstructionDecoder('browser-recovery-recorded-fixture'),liveOptions:{environment:'disabled',baseUrl:'',apiKey:''}});
