...
## User-requested regression pass: all scenarios and Cobalt

- Ran the current automatic suite: **1,117 tests in 65 files**, no failures; typecheck passed. The separate API lab suite passed **20/20**, using only local servers and mocked requests.
- Replayed the five official scenario instructions through the browser in an isolated state directory, with hosted Viseca disabled. Confirmed each full JSON and observed automatic processing of all **45 proposals**.
- Found and corrected three real decoder-coverage regressions: `Cardholder requests shoe size 43.`, `Requested product is a 27-inch monitor the cardholder chose.`, and `Pause if someone other than the cardholder appears to be driving the session`. All had existing executable constraints. Narrow source-and-rule checks now recognize them; additional qualifiers, negations and mismatched sizes stay blocked. Compiler v5 refreshes saved, unconfirmed reviews. Shoes and monitor reviews were successfully resumed without a new AI call.
- SCEN0000 with `min_order_chf = max_order_chf = 20` approved CHF 20. A second instruction with a CHF 1 ceiling declined CHF 20 and recorded no spending.
- SCEN0001 checked 10 proposals: 5 approvals, 5 declines, CHF 387.50 over the full scenario timeline. Per-order and rolling seven-day refusals were visible. SCEN0002 approved one CHF 165 pair and declined the remaining 11 proposals. SCEN0004 approved one CHF 289 monitor and declined the remaining 10, including the duplicate, extras and manipulated descriptions.
- Original SCEN0003: confirmed the CHF 165 device review, rejected Cobalt CHF 245, and left unresolved familiarity checks to expire. Cobalt CHF 248 displayed `Permissions need review` with the correct expiry label and an explicit new-setup action. After expiration the action still opened the SCEN0003 instruction editor.
- A separate SCEN0003 test permission omitted merchant familiarity and explicitly enabled device/burst checks. Cobalt CHF 248 showed exactly one confirmation button for both C15 and C16, and one click approved it. RainThread CHF 232 was manually rejected. Other proposals continued arriving while reviews were pending.
- Restarted only the isolated test server with the same saved state: the pending timer continued and the approved Cobalt purchase stayed approved. Let CHF 165 expire, verified that confirmation disappeared, then re-evaluated and approved it. Stored lock timestamps confirm a fresh 120-second review window.
- Checked actual CSS viewport widths 312, 320 and 1440: no horizontal document overflow; one card on mobile and two on desktop. The second card stayed selected across resizing. Restored the browser viewport and found no console errors.
- These tests created 57 proposals across seven isolated runs (45 official-flow proposals plus 12 targeted limit/risk cases). The user's application history was not used for test mutations. Recorded results: [wallet-regression-2026-09-19.json](test-results/wallet-regression-2026-09-19.json).
