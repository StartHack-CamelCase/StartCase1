...
export async function mountRunPage(o:Options):Promise<void> {
 ...
 let scrollAnchor:{id:string;relativeOffset:number}|null=null;
 ...
 function syncTimers(){
  for(const [id,{element}] of cards){
   ...
   if(timer){const seconds=confirmationSeconds(timer.dataset['expiresAt'],now());const value=countdownText(seconds);const confirmation=timer.dataset['reviewType']!=='resolution';const label=seconds===0?(confirmation?'Confirmation expired':'Request expired'):(confirmation?'Time left to confirm':'Time until this request expires');
    ...
    element.querySelectorAll<HTMLButtonElement>('[data-review-action]').forEach(button=>button.disabled=actionBlocked(id)||seconds===null||seconds===0||!allowsPurchaseResponse(view));
   }
  ...
  root.querySelector<HTMLButtonElement>('#revoke-wallet')!.disabled=pending.size>0||view.status==='revoked'||view.mandate_status==='revoked';
 }
 function controls(){
  ...
  // Layout can change before ResizeObserver runs. Keep the previous anchor
  // during that interval, even if a scroll event or a poll updates controls.
  if(Math.abs(bounds.width-railWidth)<0.5){
   if(restoredScrollLeft!==null&&Math.abs(rail.scrollLeft-restoredScrollLeft)>=0.5)restoredScrollLeft=null;
   if(restoredScrollLeft===null){const card=all[closest];if(card){const rect=card.getBoundingClientRect();scrollAnchor={id:card.dataset['purchase']!,relativeOffset:rect.width?(rect.left-bounds.left)/rect.width:0};}}
  }
 ...
 function scrollToCard(index:number){const card=[...cards.values()][index]?.element;if(!card)return;restoredScrollLeft=null;const left=rail.scrollLeft+card.getBoundingClientRect().left-rail.getBoundingClientRect().left;rail.scrollTo({left,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});}
 ...
 function cardMarkup(p:WalletRunView['purchases'][number],recovery:PendingOperation|null):string {
  const markup=renderPurchaseCard(p,view.mode,view.mandate_status==='revoked'?'revoked':view.status,view.scenario_id);if(!recovery)return markup;
 ...
 function update(v:WalletRunView){
  ...
  text('#run-status',runStatusText(v.status)+(v.mandate_status&&v.mandate_status!=='active'&&v.status!==v.mandate_status?' '+runStatusText(v.mandate_status):''));
 ...
 root.addEventListener('click',event=>{
  ...
  if(button.matches('.decline-purchase')&&allowsPurchaseResponse(view))void perform(p.authorization_id,()=>view.mode==='local'?o.mutate(`/api/simulations/${o.runId}/authorizations/${p.authorization_id}/cancel`,{}):o.mutate(`/api/wallet/runs/${o.runId}/human-responses`,{authorization_id:p.authorization_id,decision:'decline',answers:[]}));
 },{signal:listeners.signal});
 ...
 resize=new ResizeObserver(()=>{
  const bounds=rail.getBoundingClientRect(),changed=Math.abs(bounds.width-railWidth)>=0.5;
  if(changed&&scrollAnchor){
   const card=cards.get(scrollAnchor.id)?.element;
   if(card){const rect=card.getBoundingClientRect();rail.scrollTo({left:rail.scrollLeft+rect.left-bounds.left-scrollAnchor.relativeOffset*rect.width,behavior:'instant'});
    // At the end of a two-card rail the requested card cannot align left.
    // Retain its identity until navigation, rather than adopting its neighbour.
    restoredScrollLeft=rail.scrollLeft;
   }
  }
  railWidth=bounds.width;controls();
 });resize.observe(rail);
